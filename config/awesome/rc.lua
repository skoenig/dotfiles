...
-- Map tags for disconnecting from external monitor 1-to-1 to the tags with same name on the native display.
tag.connect_signal("request::screen", function(t)
	local source = t.screen
	if source.tags_relocated then return end
	source.tags_relocated = true

	local target
	for s in screen do
		if s ~= source and awful.tag.find_by_name(s, t.name) then
			target = s
			break
		end
	end
	...
	local output = next(source.outputs)
	for _, c in ipairs(client.get(source)) do
		local names, tags = {}, {}
		for _, old_tag in ipairs(c:tags()) do
			local new_tag = awful.tag.find_by_name(target, old_tag.name)
			if old_tag.screen == source and new_tag then
				table.insert(names, old_tag.name)
				table.insert(tags, new_tag)
			end
		end
		if #tags > 0 then
			c.disconnected_screen = { output = output, tags = names }
			c.screen = target
			c:tags(tags)
		end
	end
